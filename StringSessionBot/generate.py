...
ERROR_MESSAGE = "Oops! An exception occurred! \n\n**Error** : {} " \
            "\n\nTolong Laporan ke [Support](t.me/helpforRYUU) jika eror " \
...
async def main(_, msg):
    await msg.reply(
        ...
        reply_markup=InlineKeyboardMarkup([[
            InlineKeyboardButton("Pyrogram", callback_data="pyrogram for music"),
            InlineKeyboardButton("Telethon", callback_data="telethon for Ubot")
        ]])
    )
...
async def generate_session(bot, msg, telethon=False):
    ...
    text = "**{} STRING SESSION** \n\n`{}` \n\nGenerated by @RyuuString_bot".format("TELETHON" if telethon else "PYROGRAM", string_session)
    await client.send_message("me", text)
    await client.disconnect()
    await phone_code_msg.reply("Berhasil Megambil {} string session. \n\nSilahkan cek di Pesan Tersimpan/Saved Message! \n\nBy @RYUUSHINNI".format("telethon" if telethon else "pyrogram"))
